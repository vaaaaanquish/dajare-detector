...
class TrainCoOccurrenceKana(DajareTask):
    """train dataの重複音のリストから共起頻度を取る"""
    decide_kana_data = gokart.TaskInstanceParameter()
    train_test_val = gokart.TaskInstanceParameter()

    def requires(self):
        return {
            'decide_kana_data': self.decide_kana_data,
            'train_test_val': self.train_test_val
        }

    def run(self):
        df = self.load_data_frame('decide_kana_data').reset_index(drop=True)
        df = df[df['_id'].isin(
